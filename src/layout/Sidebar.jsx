...
const TestSideBar = ({setSidebarOpen, sidebarOpen}) => {
  ...
  return (
    <div className=''> 
      ...
      className=" w-[120px] h-screen shadow-2xl fixed left-0 bg-white z-50">
        {/* NavbarBrand */}
        <div className=" bg-[#D1D5DBg] h-[13%] ">
           <img src={icon} className=' w-[90%] '/>
        </div>
        {/* Nav */} 
        <ul className=''>
                {data?.map((item) => (
                  <Link activeClass='active' onClick={handleClick} to={item.to} smooth={true} duration={500} offset={-50}>
